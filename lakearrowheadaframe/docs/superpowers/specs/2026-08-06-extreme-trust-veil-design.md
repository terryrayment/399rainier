...
- Measure every center-column raster row for both washes at 2048×1246, 768×1024, and 390×844 using Chromium screenshots with `deviceScaleFactor: 1`, opaque page compositing, and PNG output. Sample `x = floor(image.width / 2)` for each integer row from 0 through `image.height - 1`. Assert CSS geometry separately. When a bridge begins at a fractional document Y-coordinate, allow the element screenshot to contain exactly one extra raster row; do not discard or resample it. The first/last source rows own endpoint checks, and every original adjacent source-row pair participates in RGB/luminance checks.
